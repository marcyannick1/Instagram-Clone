...
export async function uploadMedia(mediaPath: any, uploadPreset: any, mediaType: string, publicId?: string, transformation?: Transformation) {
    const upload = cloudinary.v2.uploader.upload(mediaPath, {
        public_id: publicId,
...
export async function mediaExist(publicId: string): Promise<boolean> {
    const response = await cloudinary.v2.search.expression(`public_id = ${publicId}`).execute();
